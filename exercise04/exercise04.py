...
def save_gif(frames, filename):
    ...
    frame_one.save(filename + ".gif", format="GIF", append_images=frames,
               save_all=True, duration=len(frames), loop=0, fps=3)

def gym_video(policy, env, filename, nr_steps = 1000):

    env.reset()
    state, _, done, _ = env.step(env.action_space.sample())  # performs random actions to start
    frames = []
        ...
        for e in tepisodes:
            ...
            for t in range(max_t):
                ...
                if step_counter % train_frequency == 0:
                    ...
                    qvalues = torch.gather(qvalues.squeeze(0), 1, torch.from_numpy(actions.astype('int64')).to(device).unsqueeze(-1)).squeeze(1)
