...
class DuelingModel(nn.Module):
    def __init__(self, nr_actions):
        ...
        torch.nn.init.kaiming_normal_(self.conv2.weight, nonlinearity="leaky_relu")
        torch.nn.init.kaiming_normal_(self.conv3.weight, nonlinearity="leaky_relu")
        torch.nn.init.kaiming_normal_(self.value_stream.weight, nonlinearity="leaky_relu")
        torch.nn.init.kaiming_normal_(self.advantage_stream.weight, nonlinearity="leaky_relu")
...
if __name__ == "__main__":
    ...
    seed = 42
    env = make_env(seed)
    dueling_model = DuelingModel(env.action_space.n).to(device)
    env.reset()
    env.step(0)
    env.render()
    ...
    optimizer = torch.optim.AdamW(dueling_model.parameters(), lr=1e-3)
